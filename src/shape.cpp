    ...
    switch (label) {
        ...
        case 's' :
            result = (pow(l,2) - pow(s + x,2))*((2*pow(r,2)*z)/pow(pow(l,2) - pow(z,2),2) -
                                                    (7*pow(s + x - z,4)*(-105*pow(l,8)*z + 140*pow(l,6)*pow(z,3) + 210*pow(l,4)*pow(z,5) - 420*pow(l,2)*pow(z,7) +
                                                                           175*pow(z,9)))/
                                                    (pow(l,5)*pow(pow(l,2) - pow(z,2),3)*(9*pow(l,6) - 63*pow(l,4)*pow(z,2) - 21*pow(l,2)*pow(z,4) -
                                                                                                  245*pow(z,6))) - (28*pow(s + x - z,3)*(-15*pow(l,10) + 3*c*pow(l,13)*r + 18*pow(l,11)*pow(r,2) -
                                                                                                                                             105*pow(l,8)*s*z - 60*pow(l,8)*pow(z,2) - 12*c*pow(l,11)*r*pow(z,2) + 90*pow(l,9)*pow(r,2)*pow(z,2) +
                                                                                                                                             140*pow(l,6)*s*pow(z,3) + 270*pow(l,6)*pow(z,4) + 50*c*pow(l,9)*r*pow(z,4) -
                                                                                                                                             330*pow(l,7)*pow(r,2)*pow(z,4) + 210*pow(l,4)*s*pow(z,5) - 300*pow(l,4)*pow(z,6) -
                                                                                                                                             76*c*pow(l,7)*r*pow(z,6) + 350*pow(l,5)*pow(r,2)*pow(z,6) - 420*pow(l,2)*s*pow(z,7) +
...
double A_para(shape shape, double z_high, double z_low, const char label){
    double result;
    result = shape.A_Block(z_high,label)/shape.Rho(z_low) - shape.A_Block(z_low,label)/shape.Rho(z_low);
    return result;
}
double A_derivative(shape shape, double z_high, double z_low, const char label){
    double result;
    result = shape.RhoDerivative(z_low,label)/(2*pow(shape.Rho(z_low),2))*(shape.A_Block(z_high,label)-shape.A_Block(z_low,label))
